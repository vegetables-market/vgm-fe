      ...
      {isOpen && (
        <div className="w-80 h-64 bg-black/90 border border-zinc-700 rounded-lg shadow-2xl flex flex-col overflow-hidden backdrop-blur-md">
          <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800 bg-zinc-900/50">
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Auth Debug Logs</span>
            <button 
              onClick={() => setLogs([])}
          ...
          <div className="flex-1 p-2 overflow-y-auto font-mono text-[10px] text-zinc-300 space-y-1">
            {logs.length === 0 ? (
              ...
              logs.map((log, i) => (
                <div key={i} className="border-b border-zinc-900 pb-1 break-all last:border-0">
                  {log}
                </div>
              ))
            )}
        ...
        <div className="flex flex-col gap-1">
          ...
          {user && (
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="text-blue-300 truncate max-w-[100px]">{user.display_name}</span>
            </div>
          )}
