    ...
    <div className="min-h-screen bg-[#f9f8f4] flex items-center justify-center p-4 font-sans text-stone-700">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-[900px] min-h-[600px] transition-all duration-300 ${isRightPanelActive ? "right-panel-active" : ""}`}>
        ...
        <div className={`absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-transform duration-700 ease-in-out z-50 ${isRightPanelActive ? "-translate-x-full" : ""}`}>
          <div 
            className={`bg-green-600 text-white relative -left-full h-full w-[200%] transform transition-transform duration-700 ease-in-out ${isRightPanelActive ? "translate-x-1/2" : "translate-x-0"}`}
            style={{
