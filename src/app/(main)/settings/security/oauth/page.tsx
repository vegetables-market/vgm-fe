...
export default function OAuthPage() {
      ...
      case "google":
        return (
          <svg width="24" height="24" viewBox="0 0 24 24">
            ...
            <path fill="#FBBC05" d="M 5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
  ...
  const isConnected = (providerId: string) => {
    return connections.some(c => c.provider.toLowerCase() === providerId.toLowerCase());
  };

  const getConnection = (providerId: string) => {
    return connections.find(c => c.provider.toLowerCase() === providerId.toLowerCase());
  };
    ...
    <div className="oauth-page">
      ...
      <div className="provider-list">
        {availableProviders.map((provider) => {
          ...
          return (
            <div key={provider.id} className={`provider-item ${connected ? 'connected' : ''}`}>
              ...
              <div className="provider-info">
                ...
                {connected && connection?.providerEmail && (
                  <div className="provider-email">{connection.providerEmail}</div>
                )}
                {connected && connection && (
                  <div className="provider-date">連携日: {formatDate(connection.connectedAt)}</div>
                )}
              </div>
      ...
      <div className="info-box">
        <h3>外部サービス連携について</h3>
        <ul>
          <li>外部アカウントを連携すると、そのアカウントでログインできるようになります</li>
