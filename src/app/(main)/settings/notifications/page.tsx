...
export default function NotificationSettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      <h1 className="mb-6 text-2xl font-bold text-gray-800">通知設定</h1>

      {message && (
        <div
          className={`mb-4 rounded p-4 ${message.includes("失敗") ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
        >
          {message}
        </div>
      )}
      ...
      <div className="space-y-6 rounded-lg bg-white p-6 shadow-md">
        ...
        <div className="flex items-center justify-between border-b pb-4">
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              settings.favoritePriceDrop && settings.emailNotifications
                ? "bg-green-600"
                : "bg-gray-300"
            } ${!settings.emailNotifications ? "cursor-not-allowed opacity-50" : ""}`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
        ...
        <div className="flex items-center justify-between border-b pb-4">
          ...
          <button
            ...
            disabled={!settings.emailNotifications}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              settings.newMessage && settings.emailNotifications
                ? "bg-green-600"
                : "bg-gray-300"
            } ${!settings.emailNotifications ? "cursor-not-allowed opacity-50" : ""}`}
          >
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={!settings.emailNotifications}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              settings.transactionUpdates && settings.emailNotifications
                ? "bg-green-600"
                : "bg-gray-300"
            } ${!settings.emailNotifications ? "cursor-not-allowed opacity-50" : ""}`}
          >
      ...
      <div className="mt-6">
        <button
          onClick={handleSave}
          disabled={saving}
          className="w-full rounded-md bg-red-600 px-4 py-3 font-semibold text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-gray-400"
        >
          {saving ? "保存中..." : "設定を保存"}
        </button>
      </div>
