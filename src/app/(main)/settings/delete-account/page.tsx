...
export default function DeleteAccountPage() {
  ...
  useEffect(() => {
    ...
    const requestDeletion = async () => {
      try {
        const data = await fetchApi<{ success: boolean; flow_id: string; auth_type: string; message: string }>(
        ...
        if (data.auth_type === "TOTP") {
            ...
            router.push(`/challenge?type=totp&token=${data.flow_id}&action=delete_account&redirect_to=${redirectTo}`);
        } else {
             const maskedEmailParam = (data as any).masked_email ? `&masked_email=${encodeURIComponent((data as any).masked_email)}` : "";
            router.push(`/challenge?type=email&flow_id=${data.flow_id}&action=delete_account&redirect_to=${redirectTo}${maskedEmailParam}`);
        }
      } catch (err: any) {
  ...
  return (
    <div className="flex h-[80vh] w-full flex-col items-center justify-center">
       <div className="w-full max-w-md rounded-lg bg-zinc-900/50 p-8 shadow-xl border border-zinc-800">
           <h2 className="mb-6 text-center text-2xl font-bold text-white">アカウント削除の確認</h2>
           ...
           <form onSubmit={handleConfirmDelete}>
             <div className="mb-8 flex items-start justify-center gap-3 px-2">
                ...
                />
                <label htmlFor="agree-delete" className="cursor-pointer select-none text-sm text-zinc-300 leading-relaxed">
                    アカウント削除に同意します。<br/>データは復元できないことを理解しました。
                </label>
             </div>
             ...
             <div className="flex flex-col gap-4">
                 <AuthButton 
                    ...
                    isLoading={isLoading} 
                    disabled={!agreed}
                    className="w-full !border-none !bg-red-600 py-3 text-white transition-colors hover:!bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    アカウントを削除する
                </AuthButton>
