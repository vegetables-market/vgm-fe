...
export default function Home() {
  ...
  const startAutoScrollLoop = () => {
    ...
    autoScrollTimerRef.current = setInterval(() => {
      const now = Date.now();
      if (
        now - lastInteractionTimeRef.current > 4000 &&
  ...
  const handleMenuClick = (category: string) => {
    ...
    const filtered = DUMMY_ITEMS.filter(
      (item) => item.category === category || category === "Vegetables"
    );

    setModalItems(filtered.length > 0 ? filtered : DUMMY_ITEMS); // データが少なければ全部出すフォールバック
  ...
  return (
    <main className="text-stone-800 antialiased">
      ...
      <div
        id="loading-overlay"
        className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#f9f8f4] text-2xl tracking-widest text-stone-600"
      >
        HARVEST LOADING...
      </div>

      {/* 2. 商品一覧モーダル (★重要：scroll-containerの外に置くことで崩れを防ぐ) */}
      <div
        id="product-modal"
        className="fixed inset-0 z-[9999] opacity-0 invisible transition-all duration-500 flex items-center justify-center"
        // 親のonClickは削除（誤作動防止のため）
      >
        {/* ★修正1: 背景(黒い部分)に直接 onClick をつける */}
        <div
        ...
        >
          ...
          <div className="flex justify-between items-center px-8 py-6 bg-white border-b border-gray-100 z-10 sticky top-0">
            <div>
              <p className="text-xs text-green-600 font-bold tracking-widest uppercase mb-1">
                Select Item
              </p>
            ...
            >
              <span className="text-2xl text-stone-500 group-hover:text-white transition-colors">
                ×
              </span>
            </button>
          </div>

          {/* ★修正2: data-lenis-prevent を追加して、ここだけスクロール可能にする */}
          <div
            className="overflow-y-auto flex-1 p-6 md:p-10 bg-[#faf9f6]"
            data-lenis-prevent
          >
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-10">
              {modalItems.map((item) => (
                <div
                  ...
                  onClick={() =>
                    (window.location.href = `/items/detail?id=${item.id}`)
                  }
                >
                  {/* 画像エリア */}
            ...
            {modalItems.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-stone-400 opacity-50">
                <p className="font-serif text-2xl">Loading Collection...</p>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="main-content-row">
                ...
                >
                  {[
                    ...
                    <div
                      key={item}
                      className="artist cursor-pointer hover:text-green-300 transition-colors"
                ...
                <div
                  className="featured relative w-1/2 h-auto text-left"
                  style={{
                    marginLeft: "auto",
                    marginRight: "auto",
                    top: "5vh",
                ...
                >
                  <div
                    className="featured-content text-white font-serif active"
                    style={{
                      fontSize: "3vw",
                      lineHeight: "1.4",
                      opacity: 1,
